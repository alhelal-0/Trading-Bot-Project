...
class Nas100PriceActionBot(bt.Strategy):
    """
    NAS100 / USTEC / MNQ 5-minute price action bot.

    Trades the way I trade, from 10 months of journals:
    ...
    3. Entries only with the bias, only on a CLOSED strong signal candle:
       - Pullback to the 9/15 EMA zone rejected by a candle closing
         at its high (buy) / low (sell)  -> 2:1 target on trend days
       - Surprise bar -> enter on the close, 1:1 measured move
       - Double bottom / double top: two lows (highs) at the same level,
         then a strong rejection candle -> 1:1 target
       - Range day: bull signal in the lower third of the day's range,
         bear signal in the upper third -> 1:1 target
       - Choppy stretch ("avoiding zone": EMAs intertwined, price weaving
         across them) -> no trend entries until it resolves
    4. Risk 1% per trade, SL beyond the signal bar / recent swing,
       ...
       leg in the same direction on the next signal ("keep buying until
       the trend reverses"), up to max_concurrent_legs.
    ...
    params = dict(
        ...
        # Setup toggles
        enable_surprise_bar=True,        # set False to skip surprise-bar entries
                                         # (detector still classifies the day)
        ...
        chop_blocks_all=False,           # True: choppy blocks every setup, not just trend entries
        ...
        rr_trend=2.0,
        rr_counter=1.0,                  # range-day and surprise trades: 1:1
    ...
    def next(self):
        ...
        # Optionally stand aside completely in sideways/choppy conditions
